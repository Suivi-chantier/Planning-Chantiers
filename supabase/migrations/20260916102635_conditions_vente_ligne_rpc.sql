-- Suite de 20260916190000_conditions_vente_ligne.sql : RPC de modification
-- des conditions de vente d'UNE ligne (simulation + application atomique).

-- ─── 5) Cœur : évaluation (simulation) et application d'UNE ligne ──────────
...
begin
  ...
  -- Options demandées : elles doivent exister ; une option désactivée n'est
  -- acceptée que si la ligne l'utilise déjà (conservation de l'existant).
  if v_mode_coef = 'specifique' then
    if p_coefficient_id is null then raise exception 'Coefficient spécifique non sélectionné.'; end if;
    ...
    if not coef.actif and (l.mode_coefficient_ligne <> 'specifique' or l.coefficient_ligne_id is distinct from coef.id) then
      raise exception 'Le coefficient « % » est désactivé : il ne peut pas être sélectionné.', coef.libelle;
    end if;
    -- Valeur FIGÉE : celle déjà sur la ligne si c'est la même option, sinon la valeur actuelle des Réglages.
    if l.mode_coefficient_ligne = 'specifique' and l.coefficient_ligne_id = coef.id then
      v_spec_coef_valeur := l.coefficient_ligne_valeur; v_spec_coef_lib := l.coefficient_ligne_libelle;
      if not coef.actif then v_avertissements := array_append(v_avertissements, 'Coefficient « ' || coef.libelle || ' » désactivé dans les Réglages : la valeur figée sur la ligne reste utilisée.'); end if;
    else
      v_spec_coef_valeur := coef.valeur; v_spec_coef_lib := coef.libelle;
    end if;
    if v_spec_coef_valeur is null or v_spec_coef_valeur <= 0 then raise exception 'Coefficient « % » invalide (valeur nulle ou négative).', coef.libelle; end if;
  end if;
  if v_mode_taux = 'specifique' then
    if p_taux_id is null then raise exception 'Taux horaire spécifique non sélectionné.'; end if;
    ...
    if not taux.actif and (l.mode_taux_horaire_ligne <> 'specifique' or l.taux_horaire_ligne_id is distinct from taux.id) then
      raise exception 'Le taux horaire « % » est désactivé : il ne peut pas être sélectionné.', taux.libelle;
    end if;
    if l.mode_taux_horaire_ligne = 'specifique' and l.taux_horaire_ligne_id = taux.id then
      v_spec_taux_valeur := l.taux_horaire_ligne_valeur; v_spec_taux_lib := l.taux_horaire_ligne_libelle;
      if not taux.actif then v_avertissements := array_append(v_avertissements, 'Taux horaire « ' || taux.libelle || ' » désactivé dans les Réglages : la valeur figée sur la ligne reste utilisée.'); end if;
    else
      v_spec_taux_valeur := taux.taux_ht; v_spec_taux_lib := taux.libelle;
    end if;
    if v_spec_taux_valeur is null or v_spec_taux_valeur <= 0 then raise exception 'Taux horaire « % » invalide (valeur nulle ou négative).', taux.libelle; end if;
  end if;
  ...
  v_hash := public.conditions_ligne_hash(p_ligne_id, v_mode_coef, p_coefficient_id, v_mode_taux, p_taux_id);
  if p_appliquer then
  ...
  v_rc := public.conditions_ligne_resoudre(v_mode_coef, p_coefficient_id, v_spec_coef_valeur, v_spec_coef_lib,
  ...
  v_rt := public.conditions_ligne_resoudre(v_mode_taux, p_taux_id, v_spec_taux_valeur, v_spec_taux_lib,
  ...
  if p_appliquer then
    ...
    update public.profero_ouvrages_selectionnes set
      ...
      coefficient_ligne_id = case when v_mode_coef = 'specifique' then p_coefficient_id end,
      ...
      taux_horaire_ligne_id = case when v_mode_taux = 'specifique' then p_taux_id end,
      ...
      calcul_detail = v_detail || jsonb_build_object(
        ...
        'coefficient_applique', jsonb_build_object('valeur', v_coef_appl, 'source', v_rc->>'source', 'mode', v_mode_coef,
                                                   ...
                                                   'specifique', case when v_mode_coef = 'specifique' then jsonb_build_object('id', p_coefficient_id, 'valeur', v_spec_coef_valeur, 'libelle', v_spec_coef_lib) end),
        ...
        'taux_applique', jsonb_build_object('valeur', v_taux_appl, 'source', v_rt->>'source', 'mode', v_mode_taux,
                                            ...
                                            'specifique', case when v_mode_taux = 'specifique' then jsonb_build_object('id', p_taux_id, 'valeur', v_spec_taux_valeur, 'libelle', v_spec_taux_lib) end),
    ...
    values (
      ...
      v_mode_coef_avant, v_mode_coef, l.coefficient_ligne_id, case when v_mode_coef = 'specifique' then p_coefficient_id end,
      ...
      v_mode_taux_avant, v_mode_taux, l.taux_horaire_ligne_id, case when v_mode_taux = 'specifique' then p_taux_id end,
...
$$;
revoke all on function public.conditions_ligne_evaluer(uuid, uuid, text, uuid, text, uuid, boolean, integer, text, boolean, boolean) from public, anon;
grant execute on function public.conditions_ligne_evaluer(uuid, uuid, text, uuid, text, uuid, boolean, integer, text, boolean, boolean) to authenticated;

-- Simulation d'UNE ligne : aucune écriture.
create or replace function public.simuler_conditions_ligne(
  p_projet_id uuid, p_ligne_id uuid, p_mode_coefficient text, p_coefficient_id uuid, p_mode_taux text, p_taux_id uuid)
returns jsonb
language sql
security invoker
set search_path = public
as $$
  select public.conditions_ligne_evaluer(p_projet_id, p_ligne_id, p_mode_coefficient, p_coefficient_id, p_mode_taux, p_taux_id, false, null, null, false, false);
$$;
revoke all on function public.simuler_conditions_ligne(uuid, uuid, text, uuid, text, uuid) from public, anon;
grant execute on function public.simuler_conditions_ligne(uuid, uuid, text, uuid, text, uuid) to authenticated;

-- Application atomique d'UNE ligne : version + hash contrôlés, tout ou rien.
-- Le navigateur ne transmet QUE des modes et des identifiants : jamais une
-- valeur numérique, un prix ni une marge.
...
set search_path = public
as $$
  select public.conditions_ligne_evaluer(p_projet_id, p_ligne_id, p_mode_coefficient, p_coefficient_id, p_mode_taux, p_taux_id, true,
                                         p_version_attendue, p_hash_attendu, p_confirmer_prix_manuel, p_confirmer_conversion_v1);
$$;
revoke all on function public.appliquer_conditions_ligne(uuid, uuid, text, uuid, text, uuid, integer, text, boolean, boolean) from public, anon;
grant execute on function public.appliquer_conditions_ligne(uuid, uuid, text, uuid, text, uuid, integer, text, boolean, boolean) to authenticated;
