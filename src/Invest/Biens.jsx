...
function StockBiens({ profil, T=THEMES_INV.dark, initialFilter }) {
  ...
  // `open` ouvre la fiche du bien — le Dashboard l'envoyait déjà sous le nom
  // `fiche`, que cet onglet ne connaissait pas : le saut retombait sur la liste.
  useEffect(() => {
    if (!initialFilter) return;
    const cible = readNavTarget(initialFilter);

    if (cible.action === "open" && cible.id) { setFicheId(cible.id); return; }

    setFiltreStatut(""); setFiltreVille(""); setFiltreAgentImmobilier(""); setSpecialFilter(""); setSearch("");
    if (cible.key === "statut") setFiltreStatut(cible.value || "");
    if (cible.key === "a_relancer") { setSpecialFilter("a_relancer"); setSortConfig({ key:"date_relance", direction:"asc" }); }
    if (cible.key === "all") setSortConfig({ key:"created_at", direction:"desc" });
  }, [initialFilter]);
...
function FicheBien({ id, profil, onRetour, T=THEMES_INV.dark }) {
  ...
  const genererFicheBienPDF = ({ showAddress = true } = {}) => {
    // Compatibilité : certains boutons ou anciens emplacements de l’onglet fiche bien
    // appellent encore genererFicheBienPDF(). On les redirige vers la nouvelle
    // fiche client Profero pour éviter d’ouvrir l’ancien modèle.
    return genererPresentationClientPDF({ showAddress });
  };
      ...
      {ficheTab === "fiche" && (
        <>
          <div className="inv-card" style={{marginBottom:16}}>
            <div className="inv-card-hd blue" style={{justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                <button className="inv-btn inv-btn-gold inv-btn-sm" onClick={() => genererPresentationClientPDF({ showAddress:true })}>
                  <Icon as={FileText} size={12}/> Fiche client Profero
                </button>
                <button className="inv-btn inv-btn-out inv-btn-sm" onClick={() => genererPresentationClientPDF({ showAddress:false })}>
                  <Icon as={Send} size={12}/> Version communauté
                </button>
              </div>
            </div>
            <div className="inv-card-bd">
