...
export const JOURNAL_MAJ = [
  {
    ...
    pages: [],
    quoi: "Jusqu'ici, pour dire au moteur de planning qu'un ouvrier est absent, que quelqu'un va faire un travail qui n'est pas celui de son équipe, ou qu'une intervention posée à la main ne doit pas bouger, il fallait passer par plusieurs écrans techniques, et on ne voyait pas ce que cela changeait. L'assistant accepte la consigne telle qu'on la dit : « Steven est absent lundi prochain », « Kev va réaliser l'ossature placo sur ce chantier même si ce n'est pas son équipe », « J'ai programmé une intervention le 25/09 ». Il la traduit, montre ce qu'il a compris, l'enregistre sur votre accord, relance le moteur et montre la semaine avant et après. Rien n'est appliqué au planning : cette étape viendra ensuite.",
    comment: "Un bouton « Assistant » apparaît en bas à droite de toutes les pages Rénovation, pour les administrateurs seulement. Écrivez la consigne. Si un point est ambigu (quel chantier, quelle date, quelle personne), l'assistant pose la question avec des choix au lieu de deviner, et un nom qu'il ne connaît pas est signalé. Il affiche ensuite une fiche : qui, quand, quels travaux, et les points d'attention, par exemple une personne hors de l'équipe habituelle du lot, ou un vendredi normalement non travaillé (0 h), gardé comme exception. « Enregistrer et recalculer » enregistre la consigne à votre nom, puis affiche la grille de la semaine : absence hachurée, tâches déplacées encadrées avec « ← décalé de … », ancienne place en pointillés, bascule Avant / Après, fins prévisionnelles qui bougent, tâches non planifiées avec leur raison. Chaque consigne créée par l'assistant reste listée dans le panneau avec « Annuler cette consigne ». Une consigne enregistrée est aussi prise en compte par la simulation du Planning et par « La semaine qui vient » du Bilan. Une absence apparaît en plus dans Réglages → Ressources et dans la fenêtre des cellules du planning. Une intervention verrouillée affiche un cadenas et ne se déplace plus par glisser tant qu'elle n'est pas déverrouillée.",
  },
  {
    date: "2026-09-24",
    type: "nouveaute",
  ...
  },
  {
    ...
    comment: "Un ouvrier imposé sur une tâche, ou sur un lot d'un chantier précis, est maintenant placé même hors de son équipe. La proposition le signale comme exception, et les autres chantiers du même lot gardent leur équipe. S'il ne peut pas être placé (absent, déjà pris, inactif), la tâche reste non planifiée et le motif est écrit en clair. Toute tâche non planifiée porte désormais une raison. Une intervention verrouillée un jour à 0 h est conservée, avec l'avertissement « Intervention hors jours travaillés (vendredi 0 h) — exception conservée ». Une consigne d'ouvrier datée ne vaut qu'entre ses deux dates. Une consigne « de préférence » sur un ouvrier est suivie quand c'est possible, et expliquée quand elle ne l'est pas. Elle ne fait jamais entrer quelqu'un dans une équipe. Les consignes que le moteur ne sait pas appliquer (date « de préférence », priorité limitée à une période, consigne incomplète) sont refusées avec un avertissement. Tout cela s'affiche dans la liste des avertissements du panneau Simulation. Aujourd'hui, aucune consigne ni absence n'est enregistrée en base : les résultats de simulation restent les mêmes, seul le temps de calcul baisse. Sur un jeu de test fictif de taille comparable à la base, le calcul du moteur passe d'environ 30 secondes à moins d'une seconde. Rien n'est écrit dans le planning.",
  },
  {
    date: "2026-09-24",
    type: "correctif",
    titre: "Planning et bibliothèque : chaque modification sera désormais conservée, et 140 ouvrages de chantier retrouvent leur fiche d'origine (préparé, pas encore appliqué)",
    pages: [{ id: "planning", label: "Planning semaine" }, { id: "bibliotheque", label: "Biblio. ouvrages" }],
    quoi: "Deux trous dans le filet de sécurité des données. Premier trou : le planning n'avait aucun historique. Une cellule modifiée ou effacée l'était pour de bon, sans retour possible — un risque qu'on ne peut pas garder avant de livrer « Appliquer au planning », qui modifiera beaucoup de cellules d'un coup. Second trou : la bibliothèque d'ouvrages non plus. C'est ce qui a laissé 153 ouvrages de chantier rattachés à une fiche disparue. La mise à jour précédente annonçait ces liens « perdus pour de bon » : c'était vrai pour 13 d'entre eux seulement. 59 fiches avaient été supprimées le 14/09 par un script de nettoyage, qui avait gardé une copie complète de chacune. Les remettre en place rend leur fiche d'origine à 140 des 153 ouvrages de chantier, et les heures réelles de ces ouvrages comptent de nouveau dans la jauge d'échantillon des cadences.",
    comment: "Rien ne change tant que la base n'a pas été mise à jour : les scripts sont prêts et relus, ils seront appliqués dans un second temps. Une fois appliqués : toute modification ou suppression d'une cellule du planning ou d'un ouvrage de bibliothèque est gardée dans l'historique (Réglages, onglet Historique), avec son état d'avant. Les 59 fiches reviennent avec leurs identifiants d'origine, mais directement ARCHIVÉES : le ménage du 14/09 est respecté, elles n'apparaissent dans aucune liste de choix (phasage, import de devis, chiffrage). On les retrouve sur la page Bibliothèque en cochant « Afficher les archivés ». Ces fiches ayant été supprimées avant l'arrivée des coefficients de vente, elles reçoivent le coefficient et le taux horaire par défaut des Réglages (1,5 et 80 €/h aujourd'hui). Aucun devis n'utilise ces fiches, aucun prix n'est recalculé. Restent 13 ouvrages de chantier sans fiche, sur 13 chantiers : leurs 4 fiches ont été supprimées sans laisser de copie, elles restent introuvables.",
  },
  {
    date: "2026-09-24",
    type: "correctif",
  ...
  },
  {
    ...
    pages: [{ id: "alertes", label: "Alertes" }],
    quoi: "Le relevé hebdomadaire signalait déjà des anomalies, mais TOUS les chantiers en portaient au moins une, toutes au même niveau de gravité : 25 sur 25. Autant dire aucune. Impossible de savoir par où commencer. La nouvelle page ne détecte rien de nouveau : elle TRIE ce qui existait déjà, en trois niveaux. « Critique » regroupe les chantiers sur lesquels une décision change encore quelque chose : marge à terminaison négative alors que le chantier n'est pas fini, ou marge qui recule de plus de 500 € dans la semaine. « À surveiller » regroupe ceux qui consomment des heures sans avancer. « Information » regroupe le reste — dérives de lot, ouvrages sans prix — ainsi que les chantiers déjà terminés en perte, sur lesquels il n'y a plus rien à piloter. Sur le relevé de la semaine 38, cela donne 6 chantiers critiques au lieu de 25 indistincts.",
    comment: "La page « Alertes » s'ouvre depuis le menu ; une pastille rouge indique le nombre de chantiers critiques, et elle seule — pas de pastille pour les deux autres niveaux, sinon on recrée le bruit qu'on vient de supprimer. Chaque chantier occupe une seule carte, qui porte tous ses motifs, le montant en jeu quand il est connu, une phrase d'explication, et un clic ouvre le chantier. La partie « Information » est repliée par défaut, son nombre reste affiché. Point important : les chantiers dont les frais généraux ne sont pas renseignés apparaissent tous dans la liste, au niveau « Information », avec le motif « Donnée manquante : frais généraux non renseignés », et portent en plus la mention « Marge surestimée » sur leurs autres alertes — la marge affichée est trop belle, et de combien, personne ne le sait ; aucun montant corrigé n'est inventé. Sur la semaine 38, 13 chantiers sont dans ce cas. Le dépôt, qui n'est pas un chantier, n'apparaît jamais. Cet affichage est provisoire : le tableau de bord à venir réutilisera exactement le même moteur de calcul.",
  },
  {
    date: "2026-09-24",
    type: "nouveaute",
