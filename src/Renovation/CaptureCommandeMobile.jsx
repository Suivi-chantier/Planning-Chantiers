...
import {
  ...
  ChevronLeft, AlertTriangle, FileText, ShoppingCart, Truck,
} from "lucide-react";
...
// Appel IA — renvoie l'objet JSON extrait (ou jette une erreur)
async function analyseCommande(base64, mediaType) {
...
export default function CaptureCommandeMobile({ chantiers = [], T, branch = "renovation", profil = null }) {
  ...
  const nouvelleCommande = () => {
    ...
    setPhotoUrl(""); setPhotoPreview(""); setRepartir(false);
    setIaErr(""); setSaveErr("");
  ...
  const onFile = async (file) => {
    ...
    const [up, ia] = await Promise.allSettled([
      uploadPhoto(file, "commandes"),
      (async () => {
        const base64 = await fileToBase64(file);
        return analyseCommande(base64, mediaType);
      })(),
    ]);
  ...
  if (step === "setup") {
    return (
      <div style={page}>
        ...
        <input ref={fileGal} type="file" accept="image/*,application/pdf" hidden
          onChange={e => onFile(e.target.files?.[0])} />

        <button onClick={() => fileCam.current?.click()} style={{ ...btnPrimary(false), marginBottom: SPACING.sm }}>
          <Icon as={Camera} size={20} strokeWidth={2.2} /> Prendre une photo
        </button>
  ...
  if (step === "analyzing") {
    return (
      <div style={{ ...page, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh" }}>
        {photoPreview && (
          <img src={photoPreview} alt="" style={{ width: 160, height: 160, objectFit: "cover", borderRadius: RADIUS.lg, marginBottom: SPACING.lg, opacity: 0.5 }} />
        )}
        ...
        <div style={{ marginTop: 4, fontSize: FONT.sm.size, color: T.textSub }}>Lecture du numéro, fournisseur et lignes</div>
        <style>{`@keyframes spinkf{to{transform:rotate(360deg)}}.spin{animation:spinkf 1s linear infinite}`}</style>
      </div>
    );
  }
